...
def get_segformer_full_output (model_output, batch_shape):
    if hasattr(model_output, 'logits'):
        # Access the logits attribute
        logits = model_output.logits
    else:
        # Handle case where logits is not present
        logits = model_output  
    #logits = model_output.logits 

    semantic_segmentation = []
    target_sizes=[(batch_shape[2], batch_shape[3])] * batch_shape[0]    
    ...
    full_output = torch.stack(semantic_segmentation)
    return full_output